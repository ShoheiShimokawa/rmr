...
export const BookSearch = () => {
    ...
    const handleOpenDetail = (selectedBook) => {
        const isbn = selectedBook.volumeInfo.industryIdentifiers ? selectedBook.volumeInfo.industryIdentifiers.filter(
            (id) => id.type === "ISBN_13") : "";
        const author = selectedBook.volumeInfo.authors ? selectedBook.volumeInfo.authors[0] : "";
        const genre = selectedBook.volumeInfo?.categories ? selectedBook.volumeInfo.categories[0] : "";
        const book = {
    ...
    const judgeRead = (book) => {
        ...
        if (a) {
            return <Chip icon={judgeIcon(a.statusType)} label={statusTypeStr(a.statusType)} size="small" />
        }
        else {
            return null;
        }
    ...
    return (
        <div>
            ...
            <>{
                !iniSearch ? <div>initial</div> :
                    <>
                        {books &&
                            books.length >= 1 ?
                            <div className="container mx-auto space-y-3">
                                {books.map((book) => (
                                    <Card key={book.id} sx={{ maxWidth: 800 }} onClick={() => handleOpenDetail(book)} >
                                        <CardContent>
                                        ...
                                        /> */}
                                                <div className="space-y-2">
                                                    ...
                                                    <div>{book.volumeInfo.authors ? book.volumeInfo.authors[0] : ""}</div>
